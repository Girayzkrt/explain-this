...
| `OLLAMA_CLOUD_DISABLED`  | Ollama Cloud is turned off          | A cloud model was used while Ollama has cloud switched off. Turn cloud on in Ollama, or choose a model that runs on this computer.                                 |
...
**The explanation is wrong or odd.** Output comes from the model you selected — local or
cloud — and is not verified. Check anything that matters.
